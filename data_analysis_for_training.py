...
class data_analysis:
    ...
    def load_data(self,status):
        ...
        if not test:
            self.df = pd.read_csv('buy_computer_data_full.csv')
        elif test:
            self.df = pd.read_csv('buy_computer_data_hidden_70%.csv')
    ...
    def get_len_list(self):
        self.custom_but =len(self.df[self.df['buys_computer'] == 'yes'])
